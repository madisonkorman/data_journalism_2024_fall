...
**A1.** https://datawrapper.dwcdn.net/mj9zC/1/ The chart shows a drop in in-person voting during the 2020 election, likely due to covid and the numbers going back to normal for in person voting in the 2022 election.
...
md_votes$Early_pct <- (md_votes$Early / md_votes$Total) * 100
md_votes$Election_Day_pct <- (md_votes$`Election Day` / md_votes$Total) * 100
md_votes$Mail_pct <- (md_votes$Mail / md_votes$Total) * 100


write_csv(md_votes, "md_votes_pct.csv")
```
...
**A2.** Voter turnout among the 65+ age group is a significant metric, especially in early voting, because older voters often have higher turnout rates and distinct voting behaviors compared to younger age groups.https://datawrapper.dwcdn.net/nfKfr/2/ I focuses on the biggest counties in Maryland instead of all of them.

```{r}


ev_totals_65_up <- read_csv("data/ev_totals_65_up.csv")
glimpse(ev_totals_65_up)

selected_counties <- ev_totals_65_up[, c("Day", "Montgomery", "Baltimore City", "Prince George's", "Howard", "Anne Arundel")]


write_csv(selected_counties, "data/ev_turnout_selected_counties.csv")
...
**A3.** Act Blue has more donations than Winred. The donations for winred are concentrated around hagerstown while the act blue donations are concentrated around silver spring, annapolis, and columbia.For a story we could talk about when red donations are concentrated in north west md. I would want to know why the red donations are concentrated there.
https://datawrapper.dwcdn.net/oSFwb/1/
https://datawrapper.dwcdn.net/ooDyt/1/
...
zip_population <- get_acs(geography = "zip code tabulation area", 
                          variables = "B01003_001", 
                          year = 2020,
                          survey = "acs5")


zip_population <- rename(zip_population, 
                         zip = GEOID, 
                         population = estimate)
...
zip_population <- filter(zip_population, zip >= 20601 & zip <= 21930)


md_actblue$zip <- as.numeric(md_actblue$zip)
md_winred$zip <- as.numeric(md_winred$zip)

```

```{r}

md_actblue_pop <- merge(md_actblue, zip_population, by = "zip", all.x = TRUE)


md_actblue_pop$donation_rate <- (md_actblue_pop$donations / md_actblue_pop$population) * 1000


md_winred_pop <- merge(md_winred, zip_population, by = "zip", all.x = TRUE)


md_winred_pop$donation_rate <- (md_winred_pop$donations / md_winred_pop$population) * 1000

```

```{r}

write_csv(md_actblue_pop, "md_actblue_donations_per_capita.csv")


write_csv(md_winred_pop, "md_winred_donations_per_capita.csv")
