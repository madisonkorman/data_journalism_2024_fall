...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **Answer** Before there were no values, now there are values for Kamala D. Harris and Tim Walz Democratic and it is organized by county.
...
**Task** Run the following code to extract the table we want, clean up the names and remove the total row. Look at the rename() function - what do you think the numbers 2 through 6 are referring to? **Answer** This separates the values out based on the candidate. 
...
**Task** Run the following code in the gray-colored codeblock below to find the <li> tags. What does the output suggest about how many <ul> tags there are in the HTML source? **Answer** Perhaps there are 55 since that is the number at the top.
...
**Task** Run the following code in the gray-colored codeblock below to find the text inside <li> tags. Describe the output: **Answer** It gives me different column or row names with empty quotes after. And it lists all 55 of them.
...
### Task 11: Turn the contents of the <li> and <a> tags into a dataframe **Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: **Answer** This line of code added a new data set called committees_with_urls to my global enviroment. 
