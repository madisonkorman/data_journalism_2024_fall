...
**Task** Check for problems that occurred when loading the Texas precinct results by running the following code. How many problems were there, as shown by the number of rows in the output table showing errors? What do you think the problem is that R is describing? Answer below. **Answer** There are 1,640 rows, meaning 1,640 errors.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "district" field? What data type is the "precinct" field? What data type is the "mail" column? Answer below. **Answer** the data type for "precinct" is a character. while 'mail' and 'district' data type is for an interger.
...
**Task** Run the following codeblock to group by mail votes, count the number of precinct results, then sort from highest to lowest on count. How many results are there where mail is NA? What's the implication there? Answer below. **Answer**: 402345	results. There may be issues with the 402345 results.
...
**Task** Run the following codeblock to find the number of registered voters grouped by voter status reason, using summarise() to count. Are any of these worthy of exploration to you? Why or why not? Answer below. **Answer** I would look into the duplicate and perhaps 'REMOVED AFTER 2 FED GENERAL ELECTIONS IN INACTIVE STATUS' because I am unclear what that means. 
...
**Task** Run the following codeblock to glimpse the data. What data type is the "amount" field? Answer below. **Answer** The amount field is a character. 
...
**Answer** There may be an issue with the data type. For example, the data type for amount is a character when the value is a number. We should also look for duplicates in the data. There also may be incosistancies with the zip codes because some are in a different format.
...
**Answer**It added an 'x' to the besinning of it so the name did not start with a number. 
...
**Task** Run the following codeblock to attempt to add up the amount of all contributions. What does the error say when you run this code? What do you think the error means? Answer below. **Answer** The error is being caused by the sum of amount and it is because the 'amount' column is a character data type and not number.
...
**Task** Run the following codeblock to attempt to change the datatype for the amount field to a number. What is the new data type (three letter code) for amount? Answer below. **Answer** The amount column is now <dbl>, an interger.
...
**Task** Run the following codeblock to add up the amount of all contributions in this data. What is the total? Answer below. **Answer** The total is 226.
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many are there? What is the donor name? Answer below. **Answer** There are two. The donor name is Derrick Hamilton.
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. Add a description of what this code does to your reference notebook. **Answer** It renamed linenumber without the number one in the beginning. It also changed the number in the amount column so it includes decimal points. 
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, standardizing the field. Look at the difference in the result - what changed? **Answer** It standardixed the zipcodes so they are all in the same format. 
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. **Answer**There is one remaining.
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field using both the exact match method and the str_detect() method. How many mispellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook. **Answer** It corrects the spelling to 'Conowingo', renames linenumber, makes amount <dbl>, corrects the dates to be standard format.
