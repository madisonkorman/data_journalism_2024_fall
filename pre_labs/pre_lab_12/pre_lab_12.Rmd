...
**Task** Run the following code and describe the dataframe it outputs. **Answer** It is an empty data frame called releases.
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** The unique() function extracts the unique elements from the vector, returning a new vector with duplicate elements removed.
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** This function pulled out unigue words from the press releases.
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** Not necessarily because the main words used are articles which dont give us much insight. 
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** Yes, now the coee identifies more important terminoligy rather than predominantly articles. 
...
**Task** Run the following code and describe the results. **Answer** This gives me the number of times a word was used as well as the percentage, focusing in the chose stop words. It only displays the top 10 in descending order. 
...
**Task** Run the following code and describe the results. **Answer** This includes phrases rather than just words, displaying the number of times used and the percentage.
...
**Task** Run the following code and describe the results. **Answer** This shows the top 10 phrases paired with the number of times used and the percentage. 
...
**Task** Run the following code and describe the differences between the two results. Which one might be more useful for the press release data? **Answer**The first result may be better because it includes more variations of the word drive. 
...
**Task** Run the following code and describe the differences between the two results. Which one is more useful? **Answer**The second chart seems to be better because it cuts down on the words by combining similar words or the same word with different tenses. 
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** Critical as negative could be misleading because sometimes critical can rfer negatively to health or it can mean something is important so I may think twice before fully relying on sentiments.
