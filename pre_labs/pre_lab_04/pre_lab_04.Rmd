...
**Task** Run the following code to load some campaign expenditure data focused on food-related expenses in Washington, D.C. **Answer** I ran the code.
...
**Task** Run the following code to group and count records in the recipient_name column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** It did not correctly pluck out the lowercase 'w's.' Instead,the code is affecting the letters after the first letter in the word. 
...
**Task** Run the following code to group by recipient_clean and recipient_name, then filter for cities that start with W. Name three changes the refinr function made in the first 20 results. **Answer** It put everything into uppercase.It corrected the spacing. And it standardized Miller and Co
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by city_clean and city to examine cities that start with A. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** It looks very similar to the code for key_collision_merge. It uses the functions select and mutate using n_gram_merge. while key_collision_merge uses practically the same code with the main difference being key_collision_merge.
